...
int MyTrace(LPCTSTR lpszFormat, ...)
{
  ...
  vsprintf_s( buffer, 1024, lpszFormat, args );
...
void CMemLeakDetect::dumpMemoryTrace()
{
  ...
  {
    numLeaks++;
    sprintf_s(buf, MLD_MAX_NAME_LENGTH, "Memory Leak(%d)------------------->\n", numLeaks);
    AfxTrace(buf);
    //
    ...
    {
      sprintf_s(buf, MLD_MAX_NAME_LENGTH, "Memory Leak <0x%X> bytes(%d) occurance(%d) %s(%d)\n", 
        ainfo.address, ainfo.size, ainfo.occurance, ainfo.fileName, ainfo.lineNumber);
    }
    else
    {
      sprintf_s(buf, MLD_MAX_NAME_LENGTH, "Memory Leak <0x%X> bytes(%d) occurance(%d)\n", 
...
void CMemLeakDetect::symbolPaths( TCHAR* lpszSymbolPath)
{
  ...
  {
    strcat_s(lpszSymbolPath, MLD_MAX_NAME_LENGTH, _T(";"));
    strcat_s(lpszSymbolPath, MLD_MAX_NAME_LENGTH, lpszPath );
  }
...
BOOL CMemLeakDetect::symFunctionInfoFromAddresses( ULONG fnAddress, ULONG stackAddress, LPTSTR lpszSymbol )
{
  ...
  {
    _tcscpy_s(lpszSymbol, MLD_MAX_NAME_LENGTH, m_pSymbol->Name);
    return TRUE;
  }
  //create the symbol using the address because we have no symbol
  _stprintf_s(lpszSymbol, MLD_MAX_NAME_LENGTH, "0x%08X", fnAddress);
  return FALSE;
}
...
BOOL CMemLeakDetect::symSourceInfoFromAddress(UINT address, TCHAR* lpszSourceInfo)
{
  ...
  if ( SymGetLineFromAddr( m_hProcess, address, &dwDisp, &lineInfo ) )
  {
    ...
    _stprintf_s(lpszSourceInfo, MLD_MAX_NAME_LENGTH, _T("%s(%d): 0x%08X"), lineInfo.FileName, lineInfo.LineNumber, address );
    ret = TRUE;
  }
  else
  {
    ...
    if ( lpModuleInfo[0] == _T('?') || lpModuleInfo[0] == _T('\0'))
    {
      ...
      _stprintf_s(lpszSourceInfo, MLD_MAX_NAME_LENGTH, _T("0x%08X"), lpModuleInfo, address );
    }
    else
    {
...
BOOL CMemLeakDetect::symModuleNameFromAddress( UINT address, TCHAR* lpszModule )
{
  ...
  {
    _tcscpy_s(moduleInfo.ModuleName, lpszModule);
    ret = TRUE;
  }
  else
  {
