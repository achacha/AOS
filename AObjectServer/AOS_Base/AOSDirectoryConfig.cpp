...
const AString AOSDirectoryConfig::CLASS("class");
...
AOSDirectoryConfig::AOSDirectoryConfig(const AString& name, ALog& log) :
  m_Command(name),
...
void AOSDirectoryConfig::emitXml(AXmlElement& target) const
{
  if (target.useName().isEmpty())
    target.useName().assign(ASW("AOSDirectoryConfig",10));
...
void AOSDirectoryConfig::fromXml(const AXmlElement& element)
{
  ...
  while(citModule != nodes.end())
  {
    ...
    if ((*citModule)->getAttributes().get(CLASS, strClass) && !strClass.isEmpty())
    {
      ...
      m_Modules.push_back(AOSDirectoryConfig::MODULE_INFO());
      AOSDirectoryConfig::MODULE_INFO& info = m_Modules.back();
...
const AOSDirectoryConfig::MODULES& AOSDirectoryConfig::getModuleInfoContainer() const
{
  return m_Modules;
}

const AXmlElement& AOSDirectoryConfig::getInputParams() const
{
  return m_InputParams;
}

const AXmlElement& AOSDirectoryConfig::getOutputParams() const
{
  return m_OutputParams;
}

const AString& AOSDirectoryConfig::getCommandName() const
{ 
  return m_Command;
}

bool AOSDirectoryConfig::isEnabled() const
{
  return m_Enabled;
}

bool AOSDirectoryConfig::isSession() const
{
  return m_Session;
}

bool AOSDirectoryConfig::isForceAjax() const
{
  return m_ForceAjax;
}

const AString& AOSDirectoryConfig::getInputProcessorName() const
{ 
  return m_InputProcessor;
}

const AString& AOSDirectoryConfig::getOutputGeneratorName() const 
{ 
  return m_OutputGenerator;
}

AString& AOSDirectoryConfig::useCommandName()
{
  return m_Command;
}
