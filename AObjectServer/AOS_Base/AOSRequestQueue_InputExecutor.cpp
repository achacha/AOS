...
#include "AOSRequestQueue_InputExecutor.hpp"
...
const AString& AOSRequestQueue_InputExecutor::getClass() const
{
  static const AString CLASS("AOSRequestQueue_InputExecutor");
  return CLASS;
}

void AOSRequestQueue_InputExecutor::addAdminXml(AXmlElement& eBase, const AHTTPRequestHeader& request) const
{
  ...
  while (cit != m_Executors.end())
  {
    ...
    addProperty(
      eThread,
      ASW("isIdle",6),
      AString::fromBool(pthread->getBitArray().isBit(AOSRequestQueue_InputExecutor::BITNUM_IDLE))
    );
    addProperty(
      eThread,
      ASW("state",5),
    ...
    if (pthread->getBitArray().isBit(AOSRequestQueue_InputExecutor::BITNUM_WALKSTACK))
    {
      ...
      pthread->useBitArray().clearBit(AOSRequestQueue_InputExecutor::BITNUM_WALKSTACK);
    }
...
void AOSRequestQueue_InputExecutor::processAdminAction(AXmlElement& base, const AHTTPRequestHeader& request)
{
  ...
  if (propertyName.equals("AOSRequestQueue_InputExecutor.thread_count"))
  {
    AString value;
  ...
  else if (0 == propertyName.find("AOSRequestQueue_InputExecutor.AThread["))
  {
      ...
      while (cit != m_Executors.end())
      {
        ...
        if (pthread->getId() == thread_id)
        {
          ...
          else if (0 == actionName.find("WalkStack"))
          {
            ...
            pthread->useBitArray().setBit(AOSRequestQueue_InputExecutor::BITNUM_WALKSTACK);
            break;
          }
          else
          {
...
AOSRequestQueue_InputExecutor::AOSRequestQueue_InputExecutor(
  ...
  AOSRequestQueueInterface(pForward, pBack),
  m_Services(services),
...
void AOSRequestQueue_InputExecutor::setExecutorCount(u1 count /* = 3 */)
{
  m_executorCount = count;
}

u4 AOSRequestQueue_InputExecutor::_threadprocDispatcher(AThread& thread)
{
  AOSRequestQueue_InputExecutor *pThis = static_cast<AOSRequestQueue_InputExecutor *>(thread.getThis());
  AASSERT(NULL, pThis);
    ...
    while(thread.isRun() || pThis->m_Executors.size() > 0)
    {
      ...
      if (diff > 0)
      {
        ...
        while (diff > 0)
        {
          ...
          pthread->useBitArray().setBit(AOSRequestQueue_InputExecutor::BITNUM_IDLE, true);         //a_Starts off as idle
  ...
  {
    pThis->m_Services.useLog().add(ASWNL("Unknown exception caught in AOSRequestQueue_InputExecutor::threadprocDispatcher"), ALog::EXCEPTION);
    thread.setRunning(false);
    throw;
  }
  thread.setRunning(false);
...
u4 AOSRequestQueue_InputExecutor::_threadprocExecutor(AThread& thread)
{
  thread.setRunning(true);

  AOSRequestQueue_InputExecutor *pThis = static_cast<AOSRequestQueue_InputExecutor *>(thread.getParameter());
  ...
  try
  {
    ...
    while(thread.isRun())
    {
      if (pRequest = pThis->_next()) 
      {
        if (!ADebugDumpable::isPointerValid(pRequest))
        {
          AString error("AOSRequest pointer is invalid: ");
          error.append(AString::fromPointer(pRequest));
        ...
        context.setExecutionState(ASW("Processing request",18));
        bool processed = pThis->_processRequest(context);
        ...
        if (!processed)
        {
          ...
          try 
          {
            ...
            context.useAFile().close();
            context.setExecutionState(ASW("Waiting for request",19));
            context.setContextThreadIdle(true);   //a_Thread is now marked idle
            context.setThread(NULL);              //a_Disassociate from thread
          }
          catch(AException& ex)
          {
        ...
        {
          context.setExecutionState(ASW("Finished processing request",27));
          str.clear();
          context.useRequestHeader().useUrl().emit(str);
  ...
  catch(...)
  {
    ...
    pThis->m_Services.useLog().add(AString("Unknown exception caught in AOSRequestQueue_InputExecutor::threadprocDispatcher"), ALog::EXCEPTION);
    thread.setRunning(false);
...
bool AOSRequestQueue_InputExecutor::_processRequest(AOSContext& context)
{
  try
  {
...
//      context.useResponseHeader().setPair(AHTTPHeader::HT_ENT_Content_Length, AString::fromSize_t(context.useOutputBuffer().getSize()));
  ...
  {
    context.addError("AOSRequestQueue_InputExecutor::_processRequest", ex);
    context.setExecutionState(ASWNL("Request processing: ")+ex.what());
    return false;
  }
  catch(...)
  {
    context.addError("AOSRequestQueue_InputExecutor::_processRequest", "Unknown exception");
...
void AOSRequestQueue_InputExecutor::stopDispatching()
{
  m_threadDispatcher.setRun(false);
}

bool AOSRequestQueue_InputExecutor::isRunning() const
{
  return m_threadDispatcher.isRunning();
}

void AOSRequestQueue_InputExecutor::add(AOSRequest *pRequest)
{
#pragma message("TODO: AOSRequestQueue_InputExecutor")
}

AOSRequest *AOSRequestQueue_InputExecutor::_next()
{
#pragma message("TODO: AOSRequestQueue_InputExecutor")
  return NULL;
}
