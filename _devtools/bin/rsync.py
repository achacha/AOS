...
def printVersion():
	print "rsync.py version 2.0.0"


def main(argv):
	cookie = Cookie()

	opts, args = getopt.getopt(argv[1:], "qrRntuCIh", ["quiet", "recursive", "relative", "dry-run", "time", "update", "cvs-ignore", "ignore-times", "help", "delete", "delete-excluded", "delete-from-source", "existing", "size-only", "modify-window=", "exclude=", "exclude-from=", "include=", "include-from=", "version"])
	for o, v in opts:
		if o in ["-q", "--quiet"]:
			cookie.quiet = 1
		if o in ["-r", "--recursive"]:
