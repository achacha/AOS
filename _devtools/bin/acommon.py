...
class Context:
  ...
  def processArgv(self, argv):
    ...
    while (len(sys.argv) > argc):
      if (sys.argv[argc] == "-clean"):
        ...
        self.setTargetPath(sys.argv[argc+1]);
        argc += 1;
  ...
  def syncPathRecursive(self, source_path, target_path):
    EXEC_RSYNC_BASE="rsync -tuC --exclude=.svn -r ";
    CMD = EXEC_RSYNC_BASE;
    if (self.verbose == 1):
      CMD = CMD + "-v ";
    if (self.dryrun == 1):
  ...
  def syncFileToOutput(self, source_file):
    if (self.win32 == 1 or self.linux32 == 1):
      self.syncFile(source_file, self.DEBUG_32);
      self.syncFile(source_file, self.RELEASE_32);
    if (self.win64 == 1 or self.linux64 == 1):
      self.syncFile(source_file, self.DEBUG_64);
      self.syncFile(source_file, self.RELEASE_64);

  def syncExecutable(self, name):
    if (self.win32 == 1):
      self.syncPathFiles(self.DEBUG_32, name+".exe", self.TARGET_DEBUG_32);
      self.syncPathFiles(self.DEBUG_32, name+".pdb", self.TARGET_DEBUG_32);
      self.syncPathFiles(self.RELEASE_32, name+".exe", self.TARGET_RELEASE_32);
      self.syncPathFiles(self.RELEASE_32, name+".map", self.TARGET_RELEASE_32);
    
    if (self.win64 == 1):
      self.syncPathFiles(self.DEBUG_64, name+".exe", self.TARGET_DEBUG_64);
      self.syncPathFiles(self.DEBUG_64, name+".pdb", self.TARGET_DEBUG_64);
      self.syncPathFiles(self.RELEASE_64, name+".exe", self.TARGET_RELEASE_64);
      self.syncPathFiles(self.RELEASE_64, name+".map", self.TARGET_RELEASE_64);
  ...
  def syncLibrary(self, name):
    if (self.win32 == 1):
      self.syncPathFiles(self.DEBUG_32, name+".dll", self.TARGET_DEBUG_32);
      self.syncPathFiles(self.DEBUG_32, name+".lib", self.TARGET_DEBUG_32);
      self.syncPathFiles(self.DEBUG_32, name+".pdb", self.TARGET_DEBUG_32);
      self.syncPathFiles(self.RELEASE_32, name+".dll", self.TARGET_RELEASE_32);
      self.syncPathFiles(self.RELEASE_32, name+".lib", self.TARGET_RELEASE_32);
      self.syncPathFiles(self.RELEASE_32, name+".map", self.TARGET_RELEASE_32);
    
    if (self.win64 == 1):
      self.syncPathFiles(self.DEBUG_64, name+".dll", self.TARGET_DEBUG_64);
      self.syncPathFiles(self.DEBUG_64, name+".lib", self.TARGET_DEBUG_64);
      self.syncPathFiles(self.DEBUG_64, name+".pdb", self.TARGET_DEBUG_64);
      self.syncPathFiles(self.RELEASE_64, name+".dll", self.TARGET_RELEASE_64);
      self.syncPathFiles(self.RELEASE_64, name+".lib", self.TARGET_RELEASE_64);
      self.syncPathFiles(self.RELEASE_64, name+".map", self.TARGET_RELEASE_64);
