#ifndef INCLUDED_AMD5Digest_HPP_
#define INCLUDED_AMD5Digest_HPP_
...
public:
  ...
  **/
  void reset();
  void update(AFile&);
  void update(const AString&, size_t inputLen = AConstant::npos);  //a_inputLen can be specified for partial buffers
  void finalize();

  //a_Get result AFTER finalize() was called
  inline const AString& getDigest() const;     //a_Binary form
  void emit(AOutputBuffer&) const;             //a_ASCII emit using ATextConverter::encodeHEX
