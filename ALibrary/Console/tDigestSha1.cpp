...
static void SHA1Transform(u4 state[5], unsigned char buffer[64]);
...
static void
SHA1Transform(u4 state[5], unsigned char buffer[64])
{
  u4 a, b, c, d, e;
  typedef union {
    unsigned char c[64];
...
void testExternal()
{
  SHA1Context *pCtx = new SHA1Context();
  SHA1Init(pCtx);
  SHA1Update(pCtx, (u1 *)"abc", 3);
  SHA1Final(pCtx);

  AString str;
  AString strDigest((const char *)pCtx->digest, SHA1_DIGEST_LENGTH);
  ATextConverter::encodeHEX(strDigest, str);
...
{
  ADigest_SHA1 sha1;
  sha1.update(ASW("abc",3));
  sha1.finalize();

  std::cout << sha1 << std::endl;
  std::cout << "a9993e364706816aba3e25717850c26c9cd0d89d" << std::endl;
}
